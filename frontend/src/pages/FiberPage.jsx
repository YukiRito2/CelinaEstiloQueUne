    ...
    <div className="App">
      ...
      <main>
        ...
        >
          {/* Telefonía móvil */}
          <section
            className="noise relative overflow-hidden bg-[#14532D] py-20 sm:py-24 text-[#ECFDF3]"
            data-testid="fiber-page-mobile"
          >
            <div
              className="absolute inset-0 bg-[radial-gradient(circle_at_80%_20%,rgba(22,163,74,0.4),transparent_55%)]"
              aria-hidden="true"
            />
            <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
              <Reveal>
                <p className="font-mono-brand text-[11px] tracking-[0.3em] uppercase text-[#A7F3D0] mb-4">
                  Telefonía móvil
                </p>
                <h2 className="font-display text-3xl sm:text-4xl font-light tracking-tight">{p.mobile.title}</h2>
                <p className="mt-4 max-w-2xl text-sm sm:text-base font-light text-[#C9F5DD] leading-relaxed">
                  {p.mobile.text}
                </p>
              </Reveal>
                ...
                {p.mobile.points.map((point, i) => (
                  <Reveal key={i} delay={i * 0.1}>
                    <div className="h-full rounded-3xl bg-white/8 border border-white/12 p-6 flex flex-col gap-4 backdrop-blur-sm">
                      <span className="inline-flex w-11 h-11 rounded-2xl bg-[#16A34A] text-white items-center justify-center">
                        <Smartphone className="w-5 h-5" />
                      </span>
                      <p className="text-sm font-light text-[#ECFDF3] leading-relaxed">{point}</p>
                    </div>
                  </Reveal>
                ))}
              </div>
              ...
              <Reveal delay={0.15}>
                <div className="mt-8 flex flex-wrap gap-2">
                  {p.operators.map((op) => (
                    <span
                      key={op}
                      className="inline-flex items-center gap-1.5 font-mono-brand text-[10px] tracking-[0.15em] px-3 py-1.5 rounded-full border border-white/20 bg-white/10 text-[#ECFDF3]"
              ...
              <Reveal delay={0.18}>
                ...
                >
                  <span className="inline-flex w-10 h-10 rounded-xl bg-[#16A34A] text-white items-center justify-center shrink-0">
                    <MapPin className="w-5 h-5" />
                  </span>
                  <div>
                    <p className="text-sm font-medium text-[#ECFDF3]">{p.mobile.simNote}</p>
                    <p className="mt-1 text-xs font-light text-[#A7F3D0]">
                ...
                {p.features.map((f, i) => {
                  ...
                  return (
                    <Reveal key={f.title} delay={i * 0.1}>
                      <div
                        className="group h-full rounded-3xl bg-white border border-[#1E2430]/8 p-8 transition-all duration-500 hover:-translate-y-2 hover:shadow-xl hover:shadow-[#16A34A]/10"
                        data-testid={`fiber-feature-${i}`}
                      >
                        <span className="inline-flex w-12 h-12 rounded-2xl bg-[#BBF7D0] text-[#15803D] items-center justify-center transition-transform duration-500 group-hover:scale-110">
          ...
          {/* FAQ acordeón */}
          <section className="py-20 sm:py-24 bg-[#DCFCE7]" data-testid="fiber-page-faq">
            <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
              <Reveal>
                <p className="font-mono-brand text-[11px] tracking-[0.3em] uppercase text-[#15803D] mb-4">FAQ</p>
                <h2 className="font-display text-3xl sm:text-4xl font-light tracking-tight text-[#1E2430]">
                  {p.faqTitle}
                </h2>
              </Reveal>
                ...
                <Accordion type="single" collapsible className="mt-10">
                  {p.faq.map((item, i) => (
                    <AccordionItem
                      key={i}
                      value={`faq-${i}`}
                      className="border-b border-[#16A34A]/15"
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="mt-10 grid sm:grid-cols-3 gap-5">
                {p.facts.map((fact, i) => (
                  <Reveal key={i} delay={i * 0.1}>
                    <div
                      className="h-full rounded-3xl bg-white border border-[#86EFAC] p-7 flex flex-col gap-4"
                      data-testid={`fiber-fact-${i}`}
                    >
                      <span className="inline-flex w-10 h-10 rounded-xl bg-[#BBF7D0] text-[#15803D] items-center justify-center">
          ...
          {/* Partner: Nasertel */}
          <section className="py-20 sm:py-24 bg-[#DCFCE7]" data-testid="fiber-page-partner">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
              <Reveal>
                <p className="font-mono-brand text-[11px] tracking-[0.3em] uppercase text-[#15803D] mb-5">
                  {p.partnerTitle}
                </p>
                {/* TODO: sustituir por el logo real de Nasertel en PNG cuando esté disponible */}
                <div
                  className="mx-auto inline-flex items-center justify-center rounded-2xl bg-white border border-[#86EFAC] px-10 py-6 shadow-sm"
                  data-testid="partner-badge-nasertel"
                >
                  <span className="font-display text-2xl sm:text-3xl font-semibold tracking-tight text-[#15803D]">
                    Nasertel
                  </span>
                </div>
                ...
                <div className="mt-6 flex flex-wrap justify-center gap-2">
                  {p.operators.map((op) => (
                    <span
                      key={op}
                      className="inline-flex items-center gap-1.5 font-mono-brand text-[10px] tracking-[0.15em] px-3 py-1.5 rounded-full border border-[#86EFAC] bg-white/70 text-[#14532D]"
                ...
                <a
                  ...
                  className="mt-6 inline-flex items-center gap-2 font-semibold text-[#15803D] link-underline"
                >
                  {p.partnerLink}
          ...
          {/* Cierre */}
          <section
            className="noise relative overflow-hidden bg-[#16A34A] py-20 sm:py-24 text-white"
              ...
              <Reveal>
                ...
                <div className="mt-8 flex flex-col sm:flex-row justify-center gap-4">
                  <a
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-full bg-white text-[#15803D] font-semibold px-8 py-4 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]"
                  >
                    <MessageCircle className="w-5 h-5" />
                    {p.ctaLabel}
                  </a>
                  <a
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-full border-2 border-white/70 text-white font-semibold px-8 py-4 transition-all duration-300 hover:bg-white hover:text-[#15803D] hover:scale-[1.02] active:scale-[0.98]"
                  >
                    {p.partnerLink}
